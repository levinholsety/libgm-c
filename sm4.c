...
EVP_CIPHER_CTX *gm_sm4_new(unsigned char *key, unsigned char *iv, int enc)
{
    EVP_CIPHER_CTX *ctx = EVP_CIPHER_CTX_new();
    if (!ctx)
    {
    ...
    if (!EVP_CipherInit_ex(ctx, EVP_sm4_cbc(), NULL, NULL, NULL, enc))
    {
        EVP_CIPHER_CTX_free(ctx);
        return NULL;
    }
...
{
    return EVP_CipherFinal(ctx, out, outlen);
}

int gm_sm4_crypt(unsigned char *out, int *outlen, const unsigned char *in, int inlen, unsigned char *key, unsigned char *iv, int enc)
{
